...
#endif // bintree_eda_h
// AQUI FUNCIÓN IMPLEMENTADA POR EL ALUMNO
struct tInfo
{
	int numAcumuladores = 0;
	int numActual = 0;
};
tInfo resolver(bintree<int> const& arbol) {
	if (arbol.empty())
		return { 0,0 };
	// Calcular el numero total y numero acumuladores de izq y der
	tInfo izq = resolver(arbol.left());
	tInfo der = resolver(arbol.right());
	// Añadir ambos
	tInfo res = { izq.numAcumuladores + der.numAcumuladores, izq.numActual + der.numActual +
	arbol.root() };
	// Si el nodo actual es acumulador
	if (izq.numActual + der.numActual == arbol.root())
		res.numAcumuladores++;
	return res;
}
int numAcumuladores(bintree<int> const& arbol) {
	// Aqui llamada a la función implementada
	return resolver(arbol).numAcumuladores;
}
void resuelveCaso() {
	bintree<int> arbol = leerArbol(-1);
	std::cout << numAcumuladores(arbol) << '\n';
}
